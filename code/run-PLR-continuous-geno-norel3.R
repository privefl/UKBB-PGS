...
is_train <- (fam$set == "train" & !fam$eid %in% rel_id)
c(sum(fam$set == "train"), sum(is_train))  # 391,124 // 349,911
...
files <- tibble(
  ...
  res_file = file.path("PLR-UKBB-geno-norel3", basename)
) %>%
  filter(!file.exists(res_file)) %>%
  filter(!basename %in% SLOW_ONES) %>%
  print(n = Inf)

bigassertr::assert_dir("PLR-UKBB-geno-norel3")
