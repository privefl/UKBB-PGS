...
\begin{figure}[htbp]
	...
	\caption{Relative predictive performance with UK (reported in figure 1) compared to PC distance with UK. PCA is computed using individuals from test 1 (Table 1), and PC distances are computed using Euclidean distance between geometric medians of the first 32 PC scores of each ancestry group (shown in figure \ref{fig:PCs-restricted}). Relative performance values are the ones reported in figure 1 of the main text.}
...
\begin{figure}[h]
	...
	\caption{Predictive performance with LDpred2 when using either HapMap3 variants (HM3) or the 1M most significant variants (top1M) for 8 phenotypes (each panel).}
...
\begin{figure}[h]
	...
	\caption{\textbf{A)} Partial correlations (and 95\% CI) achieved per phenotype (each point) and per ancestry group (each panel) when training either with LASSO or with LDpred2-auto. \textbf{B)} Focusing now on the UK panel from A), each panel represents a range of proportion of causal variants $p$ and points are colored by SNP heritability $h^2$ (estimates from LDpred2-auto).}
