...
\begin{figure}[htbp]
	...
	\caption{Relative predictive performance with UK (ratio of variance explained in one group compared to the one in the UK group) compared to PC distance with UK. PCA is computed using individuals from test 1 (Table 1), and PC distances are computed using Euclidean distance between geometric medians of the first 32 PC scores of each ancestry group (shown in figure \ref{fig:PCs-restricted}). Relative performance values are the ones reported in figure 2 of the main text.}
