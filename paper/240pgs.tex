...
Before having access to large genetic datasets such as the UK Biobank, using a single individual-level dataset to derive polygenic scores was usually pointless because of the small sample sizes available.
...
A major concern about PGS is that they usually transfer poorly to other ancestries, i.e.\ a PGS derived from individuals of European ancestry is not likely to predict as well in individuals of African ancestry.
...
Individual-level data from the UK Biobank offer an opportunity to further investigate this problem of PGS portability in a more controlled setting.
...
Finally, we compare penalized regression to LDpred2-auto, and show that summary statistics methods can be powerful methods to apply to individual-level data as well.
...
We divide this data in eight ancestry groups.
We use 391,124 individuals of UK ancestry to train polygenic scores based on individual-level genotypes and phenotypes, and assess prediction in remaining individuals from eight ancestry groups (Table \ref{tab:size-sets}).
...
We propose two solutions, either relying on projection of PCs to reference populations such as the 1000 Genomes Project, or by directly using internal data.
