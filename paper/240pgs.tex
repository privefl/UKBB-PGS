...
Then, for inferring the ``Ashkenazi Jew'' ancestry group, we use the first solution, projecting UKBB individuals onto the PCA space of a reference dataset composed of many Jew and non-Jew individuals \cite[]{behar2013no}. 
...
Figure \ref{fig:lasso-ancestry} presents the results when fitting penalized regression using a training set composed of UK individuals only and testing in nine different ancestry groups. Averaged over {\NBTRAIT} phenotypes, compared to prediction performance in individuals of UK ancestry, relative predictive ability in terms of partial-$r^2$ (Methods) is only 93.8\% in Poland, 85.6\% in Italy, 72.2\% in Iran, 64.7\% in India, 48.6\% in China, 25.2\% in the Caribbean, 18\% in Nigeria, and 85.7\% for the Ashkenazi Jews.
...
We then compute the robust center (geometric median) of the Ashkenazi Jew reference individuals, and compute the PC distance to this center for all projected UKBB individuals.
Based on visual inspection of the histogram of these distances and on the fact that the closest non-Ashkenazi Jew reference individual, an Italian Jew (Figure \ref{fig:jew-ref}), is at distance 12.7, we use a threshold of 12.5 under which to assign to the ``Ashkenazi Jew'' ancestry group.
1709 unrelated individuals are assigned to this group.
Note that, within the already defined eight ancestry groups, the closest individual to this new group belongs to the Italian group, and is at distance 17.3, therefore this new ``Ashkenazi Jew'' group is not overlapping with any of the other groups.
