...
Here, we examine the extent to which polygenic risk scores are transferable between ancestries. We apply a computationally efficient implementation of penalized regressions \cite[]{prive2019efficient} to the UK Biobank data to derive PGS for {\NBTRAIT} traits using the UK Biobank genetic and phenotypic data only.
...
[PUT AFTER? MOVE TO MAIN TEXT + ADD SUPPLEMENTARY ANALYSES]Overall, relative predictive performance decreases approximately linearly with PC distance to UK (Figure \ref{fig:ratio-dist}).
...
[ADD NEW LDPRED2-LARGE RESULTS HERE]
...
We recall that Caribbean people are mostly admixed between African, European and South Asian ancestries [REF TO PC SCORES FIGURE], which are all represented in the training set 2 here.
In figure \ref{fig:lasso-multi}, we investigate nine phenotypes: breast cancer (phecode: 174.1), prostate cancer (186), type-2 diabetes (250.2), hypertension (401), CAD (411.4), skin tone, total bilirubin concentration, lipoprotein A concentration, and years of education.
