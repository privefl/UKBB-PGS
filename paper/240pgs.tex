...
Individual-level data from the UK Biobank offer an opportunity to further investigate this question in a more controlled setting.
...
Here, we examine the extent to which polygenic risk scores are transferable between ancestries. We apply a computationally efficient implementation of penalized regression \cite[]{prive2019efficient} to the UK Biobank data to derive PGS for {\NBTRAIT} traits using the UK Biobank genetic and phenotypic data only.
...
[ADD SOMETHING ABOUT THE METHOD FOR DEFINING GROUPS HERE]
These results are presented at a finer scale than the usual continental level and allows us to show that prediction already drops within Europe (for East and South Europe).
...
[REPLACE BY NOW EXPLAINING IN THIS PAPER]We divide this data in 8 ancestry groups following \cite{prive2020ancestry}.
...
\begin{figure}[htb]
	\centerline{\includegraphics[width=0.85\textwidth]{ratio-dist}}
...
We then try to use more than HapMap3 variants for training; we use more than 8M common variants (Methods). 

[ADD NEW LDPRED2-LARGE RESULTS HERE. DO I HAVE APOB? LINK WITH RESULTS FROM ZOOMED GWAS]
...
[Paul: I guess the main take-homes here are: 
1. Portability problem is just as strong within same cohort
2. Portability problem highlighted here at high-resolution (country-level)
3. Drop-off in portability already seen within Europe, and worldwide is proportional to ?PC distance?
4. Results similar whether PRS derived from summary stats or ind-level data
5. Using target ancestries in the training data doesn?t seem to improve power much (really not sure about this conclusion though?!)]

Here, we have shown a poor portability of PGS to other populations, in agreement to what has been previously reported.
...
We showcase two methods to build polygenic scores when large individual-level data is available. Although LDpred2-auto is a method based on  summary statistics, it provides good predictive performance for individual-level data compared to penalized regression.
...
As for LDpred2, it currently cannot be run using 8M variants, but we are investigating two ways to make this possible in future work [CHANGE].
